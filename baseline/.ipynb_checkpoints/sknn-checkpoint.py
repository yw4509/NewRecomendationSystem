...
class SessionKNN:
    ...
    -------------------------
    other_variable_1: bool
        whether add the feature latitude and longitude
    other_variable_2: bool
        whether add the dummay feature desktop, mobile and tablet
    lat: float
        latitude of the region
    long: float
        longitude of the region
    desktop: int (0 or 1)
        whether session is on desktop or not
    mobile: int (0 or 1)
        whether session is on mobile or not
    tablet: int (0 or 1)
        whether session is on tablet or not    
    '''
    ...
    def __init__(self, k, sample_size=1000, sampling='recent', similarity='cosine', remind=False, pop_boost=0,
        ...
        # add 
        self.use_1=other_variables_1
        self.use_2=other_variables_2
        self.lat=lat
        self.long=long
        ...
        self.session_lat=dict()
        self.session_long=dict()
        self.session_desktop=dict()
        self.session_mobile=dict()
        self.session_tablet=dict()
    ...
    def fit(self, train):
        ...
        #-------add
        index_lat=train.columns.get_loc(self.lat)
        index_long=train.columns.get_loc(self.long)
        index_desktop=train.columns.get_loc(self.desktop)
        index_mobile=train.columns.get_loc(self.mobile)
        index_tablet=train.columns.get_loc(self.tablet)
        #-----------
        ...
        for row in train.itertuples(index=False):
            ...
            if row[index_lat]:
                lat=row[index_lat]
                long=row[index_long]
            desktop=row[index_desktop]
            mobile=row[index_mobile]
            tablet=row[index_tablet]
            session_items.add(row[index_item])
            self.session_desktop.update({session:desktop})
            self.session_mobile.update({session:mobile})
            self.session_tablet.update({session:tablet})
            self.session_lat.update({session:lat})
            self.session_long.update({session:long})
            #-------------
    ...
    def predict_next(self, session_id, input_item_id, predict_for_item_ids=None, skip=False, type='view', timestamp=0,lat=0,long=0,desktop=0,mobile=1,tablet=0):
        '''
        ...
        neighbors = self.find_neighbors(set(self.session_items), input_item_id, session_id,lat,long,desktop,mobile,tablet)
    ...
    def calc_similarity(self, session_items, sessions,lat,long,desktop,mobile,tablet):
        '''
        ...
        for session in sessions:
            ...
            if similarity > 0:
                #print(len(self.session_lat),len(self.session_item_map))
                if self.use_1:
                    lat_1=(self.session_lat[session]-lat)**2
                    long_1=(self.session_long[session]-long)**2
                    distance_1=(lat_1+long_1)**2
                    decay_1=pow(0.99998,distance_1)
                    neighbors.append((session, similarity*decay_1))
                if self.use_1:
                    lat_1=(self.session_lat[session]-lat)**2
                    long_1=(self.session_long[session]-long)**2
                    distance_1=(lat_1+long_1)**2
                    decay_1=pow(0.99998,distance_1)
                    desktop_1=(self.session_desktop[session]-desktop)**2
                    mobile_1=(self.session_mobile[session]-mobile)**2
                    tablet_1=(self.session_tablet[session]-tablet)**2
                    distance_2=(desktop_1+mobile_1+tablet_1)**2
                    decay_2=pow(0.99998,distance_2)
                    neighbors.append((session, similarity*decay_1*decay_2))                 
                else:
    ...
    # -----------------
    def find_neighbors(self, session_items, input_item_id, session_id,lat,long,desktop,mobile,tablet):
        '''
        ...
        possible_neighbors = self.calc_similarity(session_items, possible_neighbors,lat,long,desktop,mobile,tablet)

        possible_neighbors = sorted(possible_neighbors, reverse=True, key=lambda x: x[1])
        possible_neighbors = possible_neighbors[:100]
